...
def random_int(min: int, max: int) -> int:
    ...
    ----------
    min : int
        Minimum value to be included in the draw.
    max : int
...
def random_math_operation() -> str:
    ...
    -------
    str
        The string corresponding to the selected mathematical operation.
    """
...
def math_quiz():
    """
    Starts a console math quiz where 3 random small math questions are generated
    and the user has to input the answer into the console. A point for each correct answer is awarded.
    ...
    for _ in range(ROUNDS):
        ...
        # or handle exception by asking for new input
        while True:
            try: 
